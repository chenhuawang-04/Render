...
> 统计 (当前分支 `develop`, commit `979ab8a`): 约 209 个头文件, 62 个源文件, 33 个着色器 + 5 个 GLSL 头文件, 14 个示例, 73 个测试文件, 22 个基准文件, 16 个文档, 8 个脚本, 4 个工具, 1 个 CMake Presets
...
| `render_loop_host.hpp` | `RenderLoopHost<WindowSurfaceT, SwapchainT, framesInFlight>`, `RenderLoopCreateInfo`, `FrameRecordContext`, `TickResult`, `TickCode` | 主帧循环。组合 Sync + Swapchain + Command + Retire，Tick() 驱动。`AcquireFrame()` 无约束模板 (Phase12 移除 `requires` 限制)，兼容任意 Recorder 类型。Swapchain 变更自动通知。 |
| `swapchain_target_set.hpp` | `SwapchainTargetSet` | 交换链目标集。管理每帧 Swapchain Image → Framebuffer 的映射，与 FrameComposer 协调最终输出 (Phase12 移除 SceneRenderTargetSet 集成)。 |
...
| `runtime_prepare_views.hpp` | `RuntimePrepareViews` | 帧准备视图集合。替代旧版 `RuntimePrepareContext`，聚合所有运行时子系统指针并传递给 Recorder 的 PrepareFrame 回调。Phase12 移除了 `SceneRenderTargetSetPrepareView` 和 `SceneBloomPostStackPrepareView`（场景目标/后处理栈已由 RenderGraph 声明式管理）。保留 `prefer_render_graph_upload_path`/`prefer_render_graph_compute_path` 等 Graph 偏好标志。 |
...
| `scene_render_target_set.hpp` | `SceneRenderTargetSetCreateInfo`, `MakeSceneRendererBinding()` | 场景渲染目标创建信息。Phase12 移除 `SceneRenderTargetSet` 类（由 RenderGraph 声明式管理场景目标），仅保留 `SceneRenderTargetSetCreateInfo` 结构和 `MakeSceneRendererBinding` 辅助函数。 |
| `scene_bloom_post_stack.hpp` | `RenderTargetBloomRendererPrepareView` | 场景 Bloom 后处理定义。Phase12 移除 `SceneBloomPostStack` 类（Bloom 管线由 RenderGraph Native Pass Plan 管理），仅保留 `RenderTargetBloomRendererPrepareView` 等基础结构。 |
...
| `render_runtime_host.hpp` | `RenderRuntimeHost<BackendTag, framesInFlight>`, `RuntimeModulesCreateInfo`, `RuntimeTickResult`, `RuntimeTickRecorder` concept | 顶层运行时。组合所有子系统，统一 Init/Shutdown 顺序。Tick() 协调 Upload、Pipeline 编译、RenderLoop、prepare 回调。Phase12 扩展 `RuntimeTickRecorder` concept (不再限于 `FrameRecorder`/`FrameContextRecorder`，接受任意含 `PrepareFrame()` 或 `BuildRenderGraph()` 的 Recorder)；移除 `RenderTargetPoolService` 集成 (RenderTargetPool 仅通过 `RenderTargetPoolStats()` 访问统计)；移除 `SceneRenderTargetSetPrepareView`/`SceneBloomPostStackPrepareView` 分发；图录制路径 fallback throw 而非静默跳过。 |

#### 2.4.6 场景提交 (Scene Submission)

| 文件 | 类/结构 | 说明 |
|------|---------|------|
| `render_view.hpp` | `RenderView<Dim>`, `RenderViewKind`, `RenderViewDesc`, `RenderViewSet` | 渲染视图。定义场景绘制的目标/视口/清除策略，支持 active/scene/overlay/reflection/custom 等 view kind。Phase12 移除 `color_final_state`/`depth_final_state` (目标最终状态现由 RenderGraph Barrier Plan 推导)。 |
...
| 文件 | 类/结构 | 说明 |
|------|---------|------|
| `scene_recorder_2d.hpp` | `SceneRecorder2D` | 2D 场景录制器。统一编排 PreScene/Scene/Overlay 三层录制。RenderGraph-only 路径 (Phase12 移除 legacy 命令式 `Record()`）：BindFramePacket→SetGraphBuildCallback→RecordGraphPass，与 RenderGraphRuntimeService 协作。 |
| `scene_recorder_3d.hpp` | `SceneRecorder3D` | 3D 场景录制器。统一编排 Light/Shadow/Scene/PostProcess 管线。RenderGraph-only 路径 (Phase12 移除 legacy 命令式 `Record()`）：BindFramePacket→SetGraphBuildCallback→RecordGraphPass，通过 Scene3DDescriptorContract 传递共享描述符。 |
...
| `frame_composer_host.hpp` | `FrameComposerHost`, `FrameComposerHostCreateInfo`, `FrameComposerHostStats` | 帧合成宿主。通过 RenderGraph 管理最终帧合成 Pass (Tonemap Composite)。Phase12 移除 `FrameComposerTargets`、`SceneRenderTargetSet` 成员、`ConfigureSceneRenderer()`/`ResetSceneRenderer()`/`RecordTonemapPass()`（所有场景目标/录制由 RenderGraph 声明式管理）；移除 `RenderTargetPool*` 参数；仅保留 `BuildRenderGraph()` 图构建接口。 |
...
| `frame_snapshot.hpp` | `FrameSnapshot<DimT>`, `FrameViewSnapshot<DimT>`, `ResolvedFrameViewSelection`, `MakeFrameSnapshot()`, `MakeFrameViewSnapshot()`, `ComposeFrameSnapshotSignature()` | 帧快照。将从 RenderScenePacket 提取的帧数据冻结为 POD 快照，用于 RenderGraph 构建。包含 View 选择 (active/scene/overlay)、摄像机数据、背景/环境覆盖、64-bit FNV-1a 签名 (用于缓存去重)。Phase12 移除 `color_final_state`/`depth_final_state` 签名贡献 (目标最终状态由 RenderGraph 推导，不再编码于快照)。 |
...
| `render_target_pool_service.hpp` | RenderTargetPool | 渲染目标池服务。(Phase12 从主力 Runtime 中移除，RenderTargetPool 现为内部实现细节，仅通过 `Runtime::RenderTargetPoolStats()` 暴露统计) |
...
| `src/render/scene_render_target_set.cpp` | `SceneRenderTargetSet` 实现 (Phase12 大幅精简，类已移除)：仅保留 `MakeSceneRendererBinding` 辅助函数。 |
| `src/render/scene_bloom_post_stack.cpp` | `SceneBloomPostStack` 实现 (Phase12 大幅精简，类已移除)：仅保留基础结构定义。 |
| `src/render/scene_recorder_2d.cpp` | `SceneRecorder2D` 实现：RenderGraph-only 路径 (Phase12 移除 legacy `Record()`，全部录制通过 `BuildRenderGraph()`→Graph Callback→`RecordGraphPass`)。 |
| `src/render/scene_recorder_3d.cpp` | `SceneRecorder3D` 实现：RenderGraph-only 路径 (Phase12 移除 legacy `Record()`，全部录制通过 `BuildRenderGraph()`→Graph Callback→`RecordGraphPass` + `Scene3DDescriptorContract`)。 |
...
| `src/render/frame_composer_host.cpp` | `FrameComposerHost` 实现 (Phase12 大幅精简 ~314→~120 行)：RenderGraph-only 帧合成 (移除 `SceneRenderTargetSet`/`RecordTonemapPass`/`ConfigureSceneRenderer`)。 |
