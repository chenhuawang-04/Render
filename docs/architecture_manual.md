...
| `SceneRecorder2D` | 2D 场景录制器。编排 PreScene/Scene/Overlay 三层录制。支持 RenderGraph 集成：BindFramePacket→SetGraphBuildCallback→RecordGraphPass，与 RenderGraphRuntimeService 协作。保留 legacy 命令式录制路径。 |
| `SceneRecorder3D` | 3D 场景录制器。编排 Light/Shadow/Scene/PostProcess 管线。支持 RenderGraph 集成：BindFramePacket→SetGraphBuildCallback→RecordGraphPass，通过 Scene3DDescriptorContract 传递共享描述符。保留 legacy 命令式录制路径。 |
...
### 4.2 RenderGraph 声明式路径 (ExecutionPhase 驱动)

RenderGraph 路径通过 `ExecutionPhaseDriver` 管理帧内阶段，完全声明式：
