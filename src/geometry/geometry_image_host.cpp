...
void GeometryImageHost::Initialize(VulkanContext& context_,
...
void GeometryImageHost::Shutdown(VulkanContext& context_) {
    if (!initialized) {
        return;
    }
...
void GeometryImageHost::BeginFrame(VulkanContext& context_,
...
void GeometryImageHost::UploadImage(VulkanContext& context_,
    ...
    if (upload_info_.pixels == nullptr) {
        throw std::invalid_argument("GeometryImageHost::UploadImage pixels must be non-null");
    }
    if (upload_info_.width == 0U || upload_info_.height == 0U) {
        throw std::invalid_argument("GeometryImageHost::UploadImage width/height must be > 0");
    }
    if (upload_info_.bytes_per_pixel == 0U) {
        throw std::invalid_argument("GeometryImageHost::UploadImage bytes_per_pixel must be > 0");
    }
    ...
    if (upload_size_bytes > static_cast<std::uint64_t>(std::numeric_limits<VkDeviceSize>::max())) {
        throw std::overflow_error("GeometryImageHost::UploadImage upload size exceeds VkDeviceSize range");
    }
...
void GeometryImageHost::RetireImage(ImageRecord& record_,
...
void GeometryImageHost::DestroyRetiredImages(VulkanContext& context_) noexcept {
...
void GeometryImageHost::RecordImageBarrier(render::UploadHost& upload_host_,
                                           std::uint32_t frame_index_,
                                           VkImage image_,
