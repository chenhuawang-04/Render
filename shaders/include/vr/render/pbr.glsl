...
vec3 ResolvePbrF0(MaterialSample material_) {
    return mix(vec3(0.04), material_.base_color, material_.metallic);
}

vec3 ResolvePbrDiffuseColor(MaterialSample material_) {
    return material_.base_color * (1.0 - material_.metallic);
}

vec3 EvaluatePbrIblFromTerms(MaterialSample material_,
                             vec3 view_dir_,
                             vec3 diffuse_irradiance_,
                             vec3 prefiltered_specular_,
                             vec2 brdf_) {
    vec3 view_dir = normalize(view_dir_);
    vec3 normal_world = normalize(material_.normal_world);
    float n_dot_v = max(dot(normal_world, view_dir), 0.0);

    vec3 diffuse_ibl = diffuse_irradiance_ * ResolvePbrDiffuseColor(material_);
    vec3 fresnel = F_Schlick(ResolvePbrF0(material_), n_dot_v);
    vec3 specular_ibl = prefiltered_specular_ * (fresnel * brdf_.x + brdf_.y);
    return diffuse_ibl + specular_ibl;
}

vec3 EvaluatePbrDirect(MaterialSample material_,
                       vec3 view_dir_,
                       vec3 light_dir_,
                       vec3 light_radiance_) {
    vec3 N = normalize(material_.normal_world);
    ...
    vec3 F0 = ResolvePbrF0(material_);
    vec3 F = F_Schlick(F0, VoH);
    float D = D_GGX(NoH, material_.roughness);
    float Vis = V_SmithGGXCorrelated(NoV, NoL, material_.roughness);

    vec3 specular = D * Vis * F;
    vec3 kd = (vec3(1.0) - F) * (1.0 - material_.metallic);
    vec3 diffuse = kd * material_.base_color * (1.0 / k_pbr_pi);
